...
//& - 2. Obtén la suma total de todas las edades de las personas francesas.
console.log(persons.filter(person => person.country == "FR")
					.map(frances => frances.age)
					.reduce((sum, num) => sum + num, 0));

//& - 3. Obtén un array con el nombre de todas las mascotas.
let arrayMascotas = persons.map(person => person.pet.name);

//& - 4. Obtén un array con las personas que tengan gato.
let arrayDuenoGatos = persons.filter(person => person.pet.type == "gato").map(gatero => gatero.name);

//& - 5. Obtén un array con los coches de los españoles.
let arrayCoches = persons.filter(person => person.country === "ES").map(spain => cars.find(matricula => matricula.id == spain.car));

//& - 6. Obtén un array con las personas que tengan un coche de la marca Ford.

//* meto en un array los coches ford
let arrayCochesFord = cars.filter((item) => { 
						return item.brand === "Ford";
					});

//* recorro el array de coches ford (.map) 					
let personas = arrayCochesFord.map(element => {
					//* busca (.filter) por cada element si coincide las matriculas y lo mete en un array (encontrado)
					let encontrado = persons.filter((itemPerson)=>{
		 								if (itemPerson.car === element.id){
											return itemPerson;
		 								}
									});
					//* Devuelve un nuevo objeto con los datos combinados
					return {
						Titular: encontrado[0].name,
